    ...
    <div class="container-sm">
        <div class="card">
            <div class="card-body">
                <div class="tables ">
                    <div style="right: 100%">
                        <a href="{{route('news.view_create_news')}}" class="btn btn-success " style="margin-left: 82%">Thêm bài viết</a>
                        ...
                        @if(count($resultNews) > 0 )
                            @foreach($resultNews as $key => $value)
                                <tr>
                                    ...
                                    <td style="text-align: center">{{$value['news_name']}}</td>
                                    <td style="text-align: center">{{$value['news_description']}}</td>
                                    <td style="text-align: center">
                                        @if($value['status'] == 1)
                                            bài viết chưa được kích hoạt
                                        @elseif($value['status'] == 2)
                                            bài viết đã được kích hoạt
                                        @endif
                                    </td>
                                    <td style="text-align: center;padding-top: 13px">
                                        <label for="" class="switch" data-id="{{$value['id']}}" data-url="{{route('news.update_status', ['id' => $value['id']])}}">
